package com.tjoeun.picok_project.oauth;
...
//	네이버 로그인 API 등록시 발급받은 Client ID, Client Secret과 Callback URL을 입력한다.
	private final static String CLIENT_ID = "49Vp8yOM0rDdjKEYTWV4";
	private final static String CLIENT_SECRET = "nOrBmZW8e8";
	private final static String REDIRECT_URI = "http://localhost:9090/picok_project/callback";
...
//	http session에 데이터를 저장하는 메소드
	private void setSession(HttpSession session, String state) {
		session.setAttribute(SESSION_STATE, state);
	}
	
//	http session의 데이터를 가져오는 메소드
	private String getSession(HttpSession session) {
		return (String) session.getAttribute(SESSION_STATE);
	}
	
//	AccessToken을 이용하여 네이버 사용자 프로필 API 호출 메소드
